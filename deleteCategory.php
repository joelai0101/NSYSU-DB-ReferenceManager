...
if ($categoryID) {
    // First, check if there are any documents associated with this category
    $sql = "SELECT COUNT(*) FROM Documents WHERE CategoryID = ?";
    if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        if ($count > 0) {
            // There are documents in this category
            $error = "無法刪除：此分類下存在文檔。";
        } else {
            // Safe to delete the category
            $sql = "DELETE FROM Categories WHERE CategoryID = ? AND UserID = ?";
            if ($stmt = mysqli_prepare($link, $sql)) {
                mysqli_stmt_bind_param($stmt, "ii", $categoryID, $_SESSION['UserID']);
                if (mysqli_stmt_execute($stmt)) {
                    if (mysqli_stmt_affected_rows($stmt) > 0) {
                        $_SESSION['success_message'] = "分類已成功刪除。";
                        header("Location: successPage.php");
                        exit;
                    } else {
                        $error = "未找到分類或您無權刪除該分類。";
                    }
                }
                mysqli_stmt_close($stmt);
            }
    ...
    <div class="wrapper">
        ...
        <div id="content" class="content-wrapper">
            <div class="container mt-5">
                ...
                <?php endif; ?>
                <a href="browseCategories.php" class="btn btn-primary">返回分類列表</a>
            </div>
        </div>
    </div>
    <script>
